...
REM
REM  Opcion B - Carpeta de inicio con .vbs (ventana minimizada):
REM    1. Crear un archivo LanzarBackup.vbs con este contenido:
REM       CreateObject("WScript.Shell").Run "cmd /c ""C:\ruta\CronWindowsCopia.bat""", 7, False
REM    2. Pulsar Win+R, escribir shell:startup y pulsar Enter
REM    3. Copiar el archivo LanzarBackup.vbs a esa carpeta
REM    Al iniciar sesion el script arrancara minimizado.
REM
...
set "ORIGEN=C:\Users\Public\pilates\db.json"
...
:MOSTRAR_OK
REM Notificacion de exito via VBScript
echo [%date% %time%] BACKUP: %ARCHIVO_DESTINO% >> "%LOG%"
cscript.exe "%~dp0Notificar.vbs" "Backup Exitoso Tusaludpilates" "Copia de datos realizada: %ARCHIVO_DESTINO%" "ok" >nul 2>&1
goto :ESPERAR

:MOSTRAR_ERROR
REM Notificacion de error via VBScript
echo [%date% %time%] ERROR: !ERROR_MSG! >> "%LOG%"
cscript.exe "%~dp0Notificar.vbs" "Error Backup Tusaludpilates" "!ERROR_MSG!" "error" >nul 2>&1
goto :ESPERAR
